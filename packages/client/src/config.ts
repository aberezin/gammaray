...
export interface ClientConfig {
   ...
   *  don't collide (e.g. 'notesync' vs 'music'). */
  dbName: string
  ...
  /** Non-type-A collections the app also wants in the same RxDB database
   *  (e.g. notesync's bespoke `note` collection). Keyed by collection name;
   *  `schema` is an RxDB JSON schema (kept loose so apps needn't import RxDB
   *  types). */
